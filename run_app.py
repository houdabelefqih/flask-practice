...
def build_username(first_name, last_name):
    ...
    """
    return "{}".format((first_name[0]+last_name).lower())
...
def search_user():
    ...
    total = sum(1 for element in [x.lower() for x in users] if search in element)

    return 'Found {} users that match with search: "{}"'.format(total, search)
